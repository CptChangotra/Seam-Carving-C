...
result_t diff_color_test(const char* test) {
    ...
    if (res != exp_res) {
        printf("expected color diff: %u, but got %u\n", exp_res, res);
        return FAILURE;
    }
    return SUCCESS;
}
...
result_t energy_small2_test(const char* test) {
    ...
    for (uint32_t i = 0; i < w * h; i++) {
        if (energy[i] != ref_energy[i]) {
            printf("energy at %u: %u\nref energy: %u", i, energy[i],
                   ref_energy[i]);
            res = FAILURE;
            break;
        }
...
result_t min_energy_wide_1_test(const char* test) {
    ...
    result_t res = SUCCESS;
    if (col != exp_col) {
        printf("expected min energy column index %u, but got %u\n", exp_col,
               col);
        res = FAILURE;
    }
    free(energy);
...
result_t optimal_path_tall_test(const char* test) {
    ...
    for (int i = 5; i >= 0; i--) {
        if (seam[i] != exp_seam[i]) {
            printf("at row %d: expected seam column %u, but got %u", i,
                   exp_seam[i], seam[i]);
            res = FAILURE;
            break;
        }
